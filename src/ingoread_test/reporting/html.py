...
def _metric_chips(metrics: dict) -> str:
    ...
    chips = "".join(
        f"<span class='chip'><b>{_esc(k)}</b>{_fmt_value(v)}</span>" for k, v in metrics.items()
    )
...
def _document_sections(
    ...
    for doc in result.document_results:
        ...
        out.append(
            ...
            f"<span class='pill' title='mean weighted similarity'>"
            f"score {_pct(doc.mean_score)}</span>"
            f"<span class='pill'>n = {doc.total_samples}</span>{doc_marker}</div></div>"
            "<table class='grid'><thead><tr><th>Field</th><th>Type</th>"
            f"<th>Match rate</th><th>Similarity</th><th>Mean metrics</th>{delta_th}</tr></thead>"
...
def _file_detail(cp: DocumentContainerPair) -> str:
    ...
    for pair in cp.document_pairs:
        field_bits = "".join(
            f"<span class='chip {'good' if m.get('matched') else 'bad'}'>{_esc(name)}</span>"
...
def render_html(
    ...
    prev_field = (
        {
            ...
            for d in previous.document_results
    ...
    cards = "".join(
        [
            ...
            _stat_card(
                "Mean similarity",
                f"<span class='big {_rate_class(result.mean_score)}'>"
                f"{_pct(result.mean_score)}</span>",
...
<div class="wrap">
  ...
  <footer>Generated by ingoread-test · match rate = fraction of documents where
    every scored field matched · similarity = stickler's weighted field score,
    which gives partial credit.</footer>
</div>
</body></html>
"""
...
def _suite_row(outcome: DatasetOutcome, output_dir: Path) -> str:
    """One dataset's line in the suite index, linking to its own report."""
    if outcome.blocked:
        verdict = _badge(False, no="advisory" if not outcome.blocking else "blocked")
    else:
        verdict = _badge(True, yes="passed")

    rate = _bar(outcome.result.match_rate) if outcome.result else "<span class='muted'>—</span>"
    n = (
        str(sum(d.total_samples for d in outcome.result.document_results))
        if outcome.result
        else "—"
    )

    links = []
    for label, path in (("json", outcome.json_path), ("html", outcome.html_path)):
        if path:
            # Relative so the whole results dir stays portable when copied.
            try:
                href = Path(path).relative_to(output_dir.resolve())
            except ValueError:
                href = Path(path)
            links.append(f"<a href='{_esc(href)}'>{label}</a>")
    notes = "; ".join(outcome.reasons) or outcome.error or "—"
    advisory_tag = "" if outcome.blocking else " <span class='pill'>advisory</span>"
    report = " · ".join(links) or "<span class='muted'>—</span>"

    return (
        f"<tr><td><b>{_esc(outcome.name)}</b>{advisory_tag}</td>"
        f"<td>{verdict}</td><td class='bar-cell'>{rate}</td><td class='num'>{n}</td>"
        f"<td>{report}</td>"
        f"<td class='muted'>{_esc(notes)}</td></tr>"
    )
...
def render_suite_html(
    ...
    """Render the suite index: one row per dataset, one release verdict."""
    output_dir.mkdir(parents=True, exist_ok=True)
    stamp = suite.start_date.strftime("%Y%m%dT%H%M%S")
    out = output_dir / f"{stamp}__{suite.suite_name}__suite.html"

    banner = (
        "<div class='banner bad'><span class='dot'></span><b>Release blocked</b> — "
        + _esc("; ".join(reasons or []) or "a blocking dataset did not pass")
        + "</div>"
        if blocked
        else "<div class='banner good'><span class='dot'></span><b>Release allowed</b> — "
        "every blocking dataset passed its own gate.</div>"
    )
    ...
    cards = "".join(
        [
            _stat_card(
                "Macro match rate",
                f"<span class='big {_rate_class(suite.macro_match_rate)}'>"
                f"{_pct(suite.macro_match_rate)}</span>",
                "mean across datasets",
            ),
            _stat_card(
                "Micro match rate",
                f"<span class='big {_rate_class(suite.micro_match_rate)}'>"
                f"{_pct(suite.micro_match_rate)}</span>",
                "pooled over all documents",
            ),
            _stat_card("Datasets", str(len(suite.datasets))),
            _stat_card("Passed", str(suite.n_passed)),
            _stat_card("Blocked", _count_value(suite.n_blocked)),
            _stat_card("Documents scored", str(suite.total_samples)),
        ]
    )
    rows = "".join(_suite_row(o, output_dir) for o in suite.datasets)

    html_doc = f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{_esc(suite.suite_name)} · ingoread-test suite</title>
<style>{_CSS}</style></head>
<body>
<div class="wrap">
  <header>
    <div class="eyebrow">ingoread-test suite</div>
    <h1>{_esc(suite.suite_name)}</h1>
    <div class="sub"><span>{_esc(suite.start_date.strftime("%Y-%m-%d %H:%M UTC"))}</span></div>
  </header>
  {banner}
  <section class="cards">{cards}</section>
  <section><h2>Datasets <span class='count'>{len(suite.datasets)}</span></h2>
    <table class='grid'><thead><tr><th>Dataset</th><th>Verdict</th><th>Match rate</th>
    <th>Docs</th><th>Report</th><th>Notes</th></tr></thead>
    <tbody>{rows or "<tr><td colspan=6 class=muted>no datasets</td></tr>"}</tbody></table>
  </section>
  <footer>Generated by ingoread-test · a release is allowed only when every
    blocking dataset passes its own gate.</footer>
</div>
</body></html>
"""
    out.write_text(html_doc, encoding="utf-8")
